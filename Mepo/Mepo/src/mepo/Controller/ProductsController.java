...
public class ProductsController {
    ...
    void search() {
        ...
        FilteredList<Product> fillteredData = new FilteredList<>(r, b -> true);
        txtSearch.textProperty().addListener((observable, oldValue, newValue) -> {
            fillteredData.setPredicate((pro) -> {
                ...
                if (pro.getProductName().toLowerCase().indexOf(lower) != -1) {
                    return true;
                } else if (pro.getDescription().toLowerCase().indexOf(lower) != -1) {
                    return true;
                } else if (pro.getPrice().toLowerCase().indexOf(lower) != -1) {
                    return true;
                } else if (ProductImp.selectCategoryNameByID(pro.getCategoryID()).toLowerCase().indexOf(lower) != -1) {
                    return true;
                } else {
                    return false;
                }
        ...
        SortedList<Product> sortedData = new SortedList<>(fillteredData);
