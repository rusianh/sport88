...
public class HomeAdminController {
    ...
    @FXML
    private BarChart<String, Number> barChartCinema;
        ...
        try (
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    totalSold += total;
                    series.getData().add(new XYChart.Data<String, Number>(productname, totalMoneyPerProduct));
                ...
                for (XYChart.Series<String,Number> serie: barChartCinema.getData()){
                    for (XYChart.Data<String, Number> item: serie.getData()){
                        item.getNode().setOnMousePressed((MouseEvent event) -> {
                            incomePerMovie.setWrapText(true);
                           incomePerMovie.setText("TOTAL INCOME OF " + item.getXValue() + " " + Helper.formatDate(from) + " to " + Helper.formatDate(to) + " = " + item.getYValue() + " $" );
                        });
                    }
                }
    ...
    public void createPieChart(LocalDate from, LocalDate to) {
        ...
        String sql = "select  d.categoryname, COUNT(a.orderdetailID) as total  from orderdetail a, `order` b, product c, category d where a.orderID = b.orderID and a.productID = c.productID and c.categoryID = d.categoryID and b.orderdate between ? and ? group by d.categoryname order by total ;";
        try (
                Connection conn = DbHelper.getConnection()) {
            assert conn != null;
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                pieChartCinema.setTitle("THE MOST POPULAR MOVIE GENRES ");
