...
//        Checked
        ...
        tcCheck.setCellValueFactory(cell -> {
            ...
            Feedback fee = cell.getValue();
            if (property.get()) {
                fee.setSelect(Boolean.TRUE);
                FeedbackImp.update(fee);
            } else {
                fee.setSelect(Boolean.FALSE);
                FeedbackImp.update(fee);
            }
            return property;
        });
...
//        search
    ...
    void checked(ObservableList<Feedback> rs) {
        for (Feedback f : rs) {
            if (f.getSelect()) {
                f.getBox().setSelected(true);
            } else {
                f.getBox().setSelected(false);
            }
        }
    }
